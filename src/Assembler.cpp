...
int readFile(const string filename)
{
	...
	if (fin.is_open())
	{
		...
		else
		{
							...
							if (line[0] != '@')
							{
								...
								if (line.find(";") != string::npos)
								{
									if (line.find('=') == string::npos)
									{
										...
										splitCode = parser_string(line, ";", line_num);
									}
									else
									{
										...
										// split the 2nd operand based on ; to generate other operands
										 vector<string> sp = parser_string(splitCode[1], ";", line_num);
										splitCode.pop_back();
			...
			std::cerr << "Error while assembling at line " << line_num << ": " << e.what() << std::endl;
			exit(-1);
			}
			fout.close();
			fin.close();
